(function() {
    ...
    var TTS = function() {
            var a = this;
            a.responsiveVoice = null;
            a.isSWF = false;
            a.ode = document.getElementById("audioElmt");
            a.html = function html(tag, attrs, parent) {
                if (!attrs) attrs = {};
            ...
            a.speakHTML5 = function(o) {

                var audio = new Audio(),
                    position = "";

                (o.oRect.top == 9) && (o.oRect.top = 200, o.oRect.left = 0, position = "position:fixed");
                ...
                a.ode.appendChild(audio);
            }

            audio = a.ode.querySelector("audio");

                if (o.dico != "responsivevoiceorg") audio.src = !(a.isSWF) ? o.url : a.speakSWF();           
            ...
            a.toggleSound = function(audio) {
                var isPaused = audio.paused;
                audio[isPaused ? "play" : "pause"]();
                audio.parentNode.children[0].style.backgroundPosition = "0px " + (isPaused ? "-54px" : "0px");
            };
            a.removeAudio = function(e) {

                var btnk = document.getElementById("audioElmt").querySelector(".audiok"),
                    target = e.target;

                if (target == btnk || target == btnk.parentNode) {

                    return;
                } else {

                    btnk.parentNode.style.opacity = 0;

                    setTimeout(function() {
                        btnk.parentNode.remove();
                    }, 500);

                    document.body.removeEventListener("click", tts.removeAudio, true);
                }
            };
            a.ResponsiveVoice = function() {
                /*
                ...
                */
                ...
                a.speak = function(b, c, aud, e) {
                    ...
                    for (b = 0; b < f.length; b++) a.fallbackMode ? (a.fallback_playbackrate = a.def_fallback_playbackrate, c = a.selectBest([d.collectionvoice.pitch, d.systemvoice.pitch, 1]), g = a.selectBest([a.iOS9 ? 1 : null, d.collectionvoice.rate, d.systemvoice.rate, 1]), h = a.selectBest([d.collectionvoice.volume, d.systemvoice.volume, 1]), null !=
                        e && (c *= null != e.pitch ? e.pitch : 1, g *= null != e.rate ? e.rate : 1, h *= null != e.volume ? e.volume : 1), c /= 2, g /= 2, h *= 2, c = Math.min(Math.max(c, 0), 1), g = Math.min(Math.max(g, 0), 1), h = Math.min(Math.max(h, 0), 1), c = a.fallbackServicePath + "?t=" + encodeURIComponent(f[b]) + "&tl=" + (d.collectionvoice.lang || d.systemvoice.lang || "en-US") + "&sv=" + (d.collectionvoice.service || d.systemvoice.service || "") + "&vn=" + (d.collectionvoice.voicename || d.systemvoice.voicename || "") + "&pitch=" + c.toString() + "&rate=" + g.toString() + "&vol=" + h.toString(),
                        g = aud,
            ...
            a.isRVO = function(o) {
                var r = (o.dico == "responsivevoiceorg");
                if (r)
                    if (a.responsiveVoice == null) a.responsiveVoice = new a.ResponsiveVoice;
                return r;
            };
        },
        tts = new TTS;

    self.port.once("ttsDOM", function(o) {      

        tts.speakHTML5(o).then(function(audio) {

            setTimeout(function() {

               audio.parentNode.children[0].style.backgroundPosition = "0px -54px";

               (tts.isRVO(o)) ? setTimeout(function() {
                                    tts.responsiveVoice.speak(o.word, o.lang, audio, { onstart: function() {}, onend: function() {} });
                                }, 500): 
                                audio.play();
           }, 100);

            audio.parentNode.children[0].onclick = function() {
                tts.toggleSound(audio);
            };

            audio.onended = function() {
                audio.parentNode.children[0].style.backgroundPosition = "0px " + (audio.ended ? "0px" : "-54px");
            }

            document.body.addEventListener("click", tts.removeAudio, true);

        }).catch(function(audio) {            
            audio.textContent = "Error" + audio.networkState;
        })
    });
